...
const siteColumnOptions = [
  ...
  { id: "weight", label: "权重", width: "minmax(0,0.5fr)" },
  { id: "tokens", label: "令牌", width: "minmax(0,0.6fr)" },
  { id: "checkin_enabled", label: "自动签到", width: "minmax(0,0.6fr)" },
  { id: "checkin", label: "今日签到", width: "minmax(0,0.8fr)" },
...
export const SitesView = ({
  ...
  const [visibleColumns, setVisibleColumns] = useState(() =>
    loadColumnPrefs(
      "columns:sites",
      siteColumnOptions.map((column) => column.id),
    ),
  );
  ...
  const updateVisibleColumns = (next: string[]) => {
    ...
    persistColumnPrefs("columns:sites", next);
  };
